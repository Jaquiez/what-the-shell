...
impl ExecContext{    
    ...
    fn redir_right(&self,expr: &Expr) -> Option<Value>{
        let left = self.interpret_program(expr.left.as_ref().unwrap(),ExecType::Quiet).unwrap(); 
        let right = self.interpret_program(expr.right.as_ref().unwrap(),ExecType::DelayExec).unwrap(); 
    ...
    fn redir_left(&self,expr: &Expr,exec_type: ExecType) -> Option<Value>{
        let left = self.interpret_program(expr.left.as_ref().unwrap(),ExecType::DelayExec).unwrap(); 
        let right = self.interpret_program(expr.right.as_ref().unwrap(),ExecType::DelayExec).unwrap(); 
        if right[0].sym != Symbol::File {
            println!("{:#?}",right);
            println!("Expected some type of file.");
    ...
    fn double_redir_left(&self,expr: &Expr, exec_type: ExecType)-> Option<Value>{
        let left = self.interpret_program(expr.left.as_ref()
            .unwrap(),ExecType::DelayExec)
            .expect("Error when executing left"); 
        let right = self.interpret_program(expr.right.as_ref()
            .unwrap(),ExecType::DelayExec)
